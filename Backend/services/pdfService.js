...
class PDFService {
  ...
  /**
   * generate pdf for course data only
   *
   * @param {object} course course data from database
   * @param {object} options optional export context from the authenticated route
   * @param {object} options.exportedBy actor name, email, role, and visibility preference
   * @returns {promise<buffer>} pdf buffer
   */
  async generateCoursePDF(course, options = {}) {
    return generateRichCoursePDF(course, options);
  }
}
