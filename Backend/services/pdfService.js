  ...
  async generateTestResultPDF(testResult, course, user, isAdmin = false) {
    return new Promise((resolve, reject) => {
      try {
        ...
        doc.roundedRect(75, infoBoxY, doc.page.width - 150, 120, 5)
        ...
        doc.roundedRect(75, statsBoxY, doc.page.width - 150, 180, 5)
        ...
        const sortedDifficulties = Object.keys(questionsByDifficulty).sort((a, b) => {
        ...
        for (const difficulty of sortedDifficulties) {
          ...
          questions.forEach((questionData, qIndex) => {
            ...
            if (fullQuestion.questionType === 'multiple' || 
              ...
              fullQuestion.options.forEach((option, optIndex) => {
                const isCorrect = optIndex === parseInt(fullQuestion.correctAnswer);
                const optionLetter = String.fromCharCode(65 + optIndex); // A, B, C, D
                
                const optionColor = isCorrect ? '#16a34a' : '#4b5563';
                const optionPrefix = isCorrect ? '✓' : ' ';
                
                doc.fillColor(optionColor)
                   .text(
                     `   ${optionPrefix} ${optionLetter}) ${option}`,
                     {
                       indent: 30,
