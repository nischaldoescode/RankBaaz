...
const TeacherAttribution = ({ teacher, compact = false }) => {
  ...
  const avatar = teacher.profileImage?.url;
  return (
    <Link
      to={teacherProfilePath(teacher)}
      className={`inline-flex items-center gap-2 rounded-full border border-gray-200 bg-white/95 px-2.5 py-1 text-xs font-medium text-gray-700 shadow-sm transition-colors hover:border-primary/40 hover:text-primary ${
        compact ? "max-w-full" : ""
      }`}
      title={`View ${teacher.name}'s teacher profile`}
    >
      {avatar ? (
        <img
          src={avatar}
      ...
      <span className="truncate">By {teacher.name || teacher.username}</span>
    </Link>
  );
};
